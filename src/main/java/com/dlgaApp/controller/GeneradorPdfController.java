...
public class GeneradorPdfController {
	...
	public void generaIncidenciaPdf(@PathVariable("idIncidencia") final long incidenciaId,HttpServletResponse response) throws IOException {
	...
	public String pdfBase64(final long incidenciaId) throws IOException {
...
public byte[] pdfStream(final long incidenciaId) throws IOException {
   ...
   @GetMapping(value = "/pdfZip")
   public void zipPdf(HttpServletResponse response) {
	   
	LocalDate hoy = LocalDate.now();

    response.setContentType("application/octet-stream");
    response.setHeader("Content-Disposition", "attachment;filename=Incidencias_"+hoy.toString()+".Zip");


    try (ZipOutputStream zippedOut = new ZipOutputStream(response.getOutputStream())) {
        for (Incidencia i :this.incidenciaService.finfAll()) {
           

            ZipEntry e = new ZipEntry("Incidencia-"+i.getId().toString()+".pdf");
            // Configure the Zip entry, the properties of the file
            e.setTime(System.currentTimeMillis());
            // etc.
            zippedOut.putNextEntry(e);
            // And the content of the resource:
            StreamUtils.copy(this.pdfStream(i.getId()), zippedOut);
